...
public class MedicalDAO {
    ...
    // ===== ADD THIS METHOD BELOW getAll() =====
    public List<Medical> getForStudentByDepartments(int ugId, List<String> departments) throws SQLException {

        if (departments == null || departments.isEmpty()) {
            return getForStudent(ugId);
        }
        ...
        String sql = "SELECT m.*, u.full_name " +
                ...
                "JOIN undergraduates ug ON m.ug_id = ug.ug_id " +
                "JOIN users u ON ug.user_id = u.user_id " +
                "WHERE m.ug_id = ? AND u.department IN (" + placeholders + ") " +
                "ORDER BY m.submitted_at DESC";
        ...
        try (PreparedStatement ps =
            ...
            for (int i = 0; i < departments.size(); i++) {
                ps.setString(i + 2, departments.get(i));
            }
    ...
    public List<Medical> getAllByDepartments(List<String> departments) throws SQLException {
        if (departments == null || departments.isEmpty()) {
            return getAll();
        }
    ...
    public void approveMedical(int medicalId) throws SQLException {
        updateStatus(medicalId, "APPROVED");
    }

    public void rejectMedical(int medicalId) throws SQLException {
        updateStatus(medicalId, "REJECTED");
    }
}